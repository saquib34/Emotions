...
const EmotionBars = ({ emotionData }) => {
  ...
  const sortedData = [...emotionArray].sort((a, b) => b.confidence - a.confidence);
    ...
    datasets: [
      {
        ...
        backgroundColor: sortedData.map(item => emotionColors[item.emotion] || '#CCCCCC'), // Fallback color
        borderColor: sortedData.map(item => {
          const color = emotionColors[item.emotion] || '#CCCCCC'; // Fallback color
          return color.replace(')', ', 1)').replace('rgb', 'rgba').replace('rgba', 'rgb');
        }),
        borderWidth: 1,
        borderRadius: 4,
      },
